...
function renderMap() {
  ...
  ctx.beginPath();
  ctx.ellipse(px+20, py+44, 14, 5, 0, 0, Math.PI*2);
  ctx.fill();

  // Player circle
  const grad = ctx.createRadialGradient(px+16,py+14,2,px+20,py+20,22);
  ...
  ctx.setLineDash([4,4]);
  ctx.beginPath();
  ctx.arc(px+20, py+20, TILE*1.8, 0, Math.PI*2);
  ctx.stroke();
  ctx.setLineDash([]);
}
...
function renderRadar() {
  ...
  if (state.radarPokemon.length === 0) { el.innerHTML = '<div id="radar-title">주변 포켓몬</div><div style="font-size:11px;color:#aaa;text-align:center;padding:4px">없음</div>'; return; }
...
function checkProximity() {
  ...
  for (const ps of state.pokestops) {
    ...
    if (Math.hypot(sx-px,sy-py) < TILE*1.8) {
      if (Date.now()-ps.lastVisit > 300000) {
        visitPokestop(ps);
      }
    }
  }
...
function throwPokeball() {
  ...
  if (ballCount <= 0) { showNotif(`${state.selectedBall === 'pokeball' ? '포켓볼' : state.selectedBall === 'greatball' ? '수퍼볼' : '울트라볼'}이 없습니다!`); return; }
  ...
  // Animate pokeball
  const ball = document.getElementById('throwing-ball');
  const area = document.getElementById('throw-area');
  const rect = area.getBoundingClientRect();
  ball.style.left = (rect.width/2 - 20) + 'px';
  ball.style.top = (rect.height - 80) + 'px';
  ball.style.display = 'block';
  ball.style.transition = 'none';

  setTimeout(() => {
    ball.style.transition = 'left 0.5s ease-out, top 0.5s ease-out, transform 0.5s';
    ball.style.left = (rect.width/2 - 20) + 'px';
    ball.style.top = '20%';
...
// ---- SCREEN NAVIGATION ----
const SCREENS = ['map','pokedex','bag','profile'];
...
function dpadPress(dir) {
  ...
  state.keys[map[dir]] = true;
}
function dpadRelease(dir) {
  const map = { up:'ArrowUp', down:'ArrowDown', left:'ArrowLeft', right:'ArrowRight' };
  state.keys[map[dir]] = false;
}
...
function init() {
  ...
  // Throw button
  document.getElementById('btn-throw').addEventListener('click', throwPokeball);
...
window.dpadPress = dpadPress;
window.dpadRelease = dpadRelease;
